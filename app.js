...
function renderHeader() {
    ...
    sidebar.innerHTML = `
        <div class="flex flex-col items-center lg:items-stretch text-center lg:text-left gap-4 fade-in staggered-1 min-w-0 max-w-full overflow-x-hidden">
            ${avatarBlock}
            <div class="flex flex-col gap-2.5 min-w-0 w-full max-w-full overflow-x-hidden">
                <h1 class="text-xl sm:text-2xl font-semibold text-zinc-900 tracking-[-0.03em] leading-tight">
                    <a href="index.html" class="hover:text-zinc-600 transition-colors">${displayName}</a>
                </h1>
                <p class="text-[10px] sm:text-[11px] font-medium tracking-[0.12em] uppercase text-zinc-500 leading-snug">
                    ${personal.bootMessage}
                </p>
                <div class="flex flex-col gap-2 text-sm w-full">
                    <a href="mailto:${personal.email}" class="link-minimal min-h-[44px] lg:min-h-0 inline-flex items-center justify-center lg:justify-start gap-1.5 break-all">
                    ...
                    <a href="tel:${personal.phone.replace(/[()\s-]/g, '')}" class="link-minimal min-h-[44px] lg:min-h-0 inline-flex items-center justify-center lg:justify-start gap-1.5 whitespace-nowrap">
                ...
                <nav class="flex flex-wrap gap-2 justify-center lg:justify-start pt-1 min-w-0 max-w-full overflow-x-hidden" aria-label="${onBlogPage ? 'Site navigation' : 'Social links'}">
                    ${onBlogPage ? `
                    ...
                    ${personal.social.github ? `
                        <a href="${personal.social.github}" target="_blank" rel="noopener noreferrer" class="pill inline-flex items-center justify-center gap-2 no-underline max-sm:min-h-[44px] max-sm:px-3.5 max-sm:py-2.5" title="GitHub" aria-label="GitHub profile">
                            <svg class="w-3.5 h-3.5 shrink-0" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg>
                            GitHub
                        </a>
                    ` : ''}
